...
(function () {
  ...
  // Stable conversation id per visitor — also used as website session ID for CRM linking.
      ...
      if (variant === 'bot') {
        ...
        el.innerHTML = escapeHtml(clean).replace(/(https?://[^s<]+[^s<.,!?')])/g, function (url) {
          if (url.indexOf('add-to-cart') > -1) {
            var refUrl = url + (url.indexOf('?') > -1 ? '&' : '?') + 'zazaRef=' + encodeURIComponent(conversationId);
            return '<a class="zaza-chat-checkout" href="' + refUrl + '" target="_blank" rel="noopener">🛒 Tap here to complete your order</a>';
          }
